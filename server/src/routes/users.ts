...
router.patch('/me', protect, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const allowed = [
      ...
      'ethnicity', 'religion', 'height', 'hasChildren', 'familyPlans',
      ...
      'hobbies', 'prompts', 'openingMove', 'filters',
    ];
  ...
  try {
    ...
    const url = await new Promise<string>((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: 'lockheart-dating', transformation: [{ width: 800, height: 1000, crop: 'fill', quality: 'auto' }] },
